      ...
      <v-col :xs="24" :sm="24">
        <div>
          <p class="h4 mb-1" style="display: flex; align-items: center">
            {{ $t('hi') }}, {{ name }}
            <span v-if="!isUser && userRole !== 'guest'" class="user-role ml-1">
              {{ $t('your_role') }}:
        ...
        <div>
          <a-tabs v-if="showTabs" :default-active-key="tab.toString()">
            ...
            >
              ...
              >
                <v-row>
                  <v-col md="6">
                    <v-autocomplete
                      ...
                      label="Category"
                      clearable
                      :rules="[(v) => !!v || 'The category is required']"
                      placeholder="Category"
                      :items="categories"
                      item-text="cate_category"
                      item-value="cate_id"
                    ></v-autocomplete>
                  </v-col>
                  <v-col md="3">
                ...
                <v-row>
                  ...
                  <v-col md="6">
                    <v-text-field
                      ...
                      :rules="[
                        (v) => !!v || 'The practice name is required',
                        (v) =>
                          (!!v && v.length > 0) || 'Enter at least 1 character',
                        (value) =>
                          (!!value && value.length <= 60) ||
                          'Enter a maximum of 60 characters',
                      ]"
                    >
                    </v-text-field>
                  </v-col>
                ...
                <v-row>
                  <v-col md="6" lg="4">
                    <v-text-field
                      v-model="medical_license"
                      label="Medical License"
                      placeholder="Medical license"
                      :rules="[
                        (v) => !!v || 'The Medical License is required',
                        (v) =>
                          (!!v && v.length >= 3) ||
                  ...
                  <v-col md="6" lg="4">
                    <v-text-field
                      v-model="license_state"
                      label="License State"
                      placeholder="License State"
                      :rules="[
                        (v) => !!v || 'The License state is required',
                        (v) =>
                          (!!v && v.length > 1) ||
                  ...
                  <v-col md="6" lg="4">
                    <v-text-field
                      ...
                      :rules="[
                        (v) => !!v || 'The credentials are required',
                        (v) =>
                          (!!v && v.length > 1) ||
                          'Enter at least 2 characters',
                        (v) =>
                ...
                <hr class="mb-1" />
                <p class="mb-1 h4">Practice Address</p>
            ...
            <!-- Claims -->
            <a-tab-pane v-if="isProfessional" key="8" tab="Claims" force-render>
              <div v-if="!isCompletedClaims">
                <a-alert
                  message="Please complete your Claims information"
                  :show-icon="true"
                  type="warning"
                  :banner="true"
                ></a-alert>
              ...
              <v-form
                ref="claimsForm"
                v-model="validClaimsForm"
                @submit.prevent="saveClaims"
              >
                <v-row>
                  <v-col md="6" lg="3">
                    <v-text-field
                      v-model="number25.value"
                      label="Federal Tax I.D. Number"
                      placeholder="Federal Tax I.D. Number"
                      :rules="[
                        (v) => !!v || 'The Federal Tax I.D. Number is required',
                        (v) =>
                          (!!v && v.length >= 3) ||
                          'Enter at least 3 characters',
                        (v) =>
                  ...
                  <v-col md="6" lg="3">
                    <v-text-field
                      v-model="number26.value"
                      label="Patient Account NO."
                      placeholder="Patient Account NO."
                      :rules="[
                        (v) => !!v || 'The Patient Account NO. is required',
                        (v) =>
                          (!!v && v.length >= 3) ||
                          'Enter at least 3 characters',
                        (v) =>
                  ...
                  <v-col md="6" lg="3">
                    <v-text-field
                      v-model="number31.value"
                      label="Signature Of Physicican"
                      placeholder="Signature Of Physicican"
                      :rules="[
                        (v) => !!v || 'The Patient Name is required',
                        (v) =>
                          (!!v && v.length >= 3) ||
                ...
                <v-row>
                  <v-col md="6" lg="4">
                    <v-text-field
                      v-model="number32.value"
                      label="Service Facility"
                      placeholder="Service Facility"
                      :rules="[
                        (v) => !!v || 'The Service Facility are required',
                        (v) =>
                          (!!v && v.length > 1) ||
                          'Enter at least 2 characters',
                        (v) =>
                  ...
                  <v-col md="6" lg="4">
                    <v-text-field
                      v-model="number32.a"
                      label="1 of number32"
                      placeholder="1 of number32"
                      :rules="[
                        (v) => !!v || 'The 1 of number32 is required',
                        (v) =>
                          (!!v && v.length >= 3) ||
                          'Enter at least 3 characters',
                        (v) =>
                  ...
                  <v-col md="6" lg="4">
                    <v-text-field
                      v-model="number32.b"
                      label="2 of number32"
                      placeholder="2 of number32"
                      :rules="[
                        (v) => !!v || 'The 2 of number32 is required',
                        (v) =>
                          (!!v && v.length >= 3) ||
                          'Enter at least 3 characters',
                        (v) =>
                ...
                <v-row>
                  <v-col md="6" lg="4">
                    <v-text-field
                      v-model="number33.value"
                      label="Billing Provider Info"
                      placeholder="Billing Provider Info"
                      :rules="[
                        (v) => !!v || 'The Billing Provider Info are required',
                        (v) =>
                          (!!v && v.length > 1) ||
                          'Enter at least 2 characters',
                        (v) =>
                  ...
                  <v-col md="6" lg="4">
                    <v-text-field
                      v-model="number33.a"
                      label="1 of number33"
                      placeholder="1 of number33"
                      :rules="[
                        (v) => !!v || 'The 1 of number33 is required',
                        (v) =>
                          (!!v && v.length >= 3) ||
                          'Enter at least 3 characters',
                        (v) =>
                  ...
                  <v-col md="6" lg="4">
                    <v-text-field
                      v-model="number33.b"
                      label="2 of number33"
                      placeholder="2 of number33"
                      :rules="[
                        (v) => !!v || 'The 2 of number33 is required',
                        (v) =>
                          (!!v && v.length >= 3) ||
                          'Enter at least 3 characters',
                        (v) =>
...
export default {
  ...
  data() {
    return {
      ...
      secondInsuredBirthDay: new Date(),
      insuredSign: '',
      patientSign: '',
        ...
        .then(({ data }) => {
          ...
          if (userInfo) {
            ...
            this.number5.patientTelephone =
              this.number5.patientTelephone || userInfo.user_phone_no
    ...
    async updateMedHistory(tab, data) {
      ...
      if (!this.fetchingMedHistory) {
        ...
        await this.$api
          ...
          .catch((err) => {
            this.fetchingMedHistory = false
            console.log('there was an error saving user data', err.data)
          })
      }
    },
    uploadPicture() {
