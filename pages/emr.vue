  ...
  <div class='pa-6 mh-100v'>
    ...
    <v-row>
      <v-col md="12">
        ...
        <v-btn color='success' small tile class='mb-1' :to="localePath('/new-emr')">
          <v-icon class='mr-1'>mdi-medical-bag</v-icon>
          New Encounter
        </v-btn>
        <v-btn dark small tile class='mb-1'
      ...
      <v-col class='mb-1'>
        <v-form ref="form" v-model="valid">
          <v-row>
            ...
            <v-col md="3">
              <v-text-field v-model='searchTerm' :placeholder="type === 'record' ? $t('search_pl1') : $t('search_pl2')"/>
            </v-col>
          </v-row>
        </v-form>
      </v-col>
      <v-col md="12">
        ...
        <v-data-table v-else :items="items" :headers="headers">
          ...
          <template #[`item.mere_is_draft`]="{item}">
            <span v-if="item.mere_is_draft" class="text-muted">
              [Draft]
            </span>
          </template>
          ...
          <template #[`item.actions`]="{item}">
            ...
            <div v-if='item.user_uuid && (isAdmin || isSuper || myUserId === item.mere_owner)'>
              <MedDivider></MedDivider>
              <a href='javascript:void(0)' type='old-rose' @click='printRecord(item.mere_uuid)'>
                <v-progress-circular v-if="isPdfLoading(item.mere_uuid)" indeterminate></v-progress-circular>
                <div v-else>
            ...
            <MedDivider></MedDivider>
            <span class='red--text clickable' @click='deleteItem(item.mere_uuid)'>{{ $t('delete') }}</span>
          </template>
          <template #[`item.owner_name`] = "{item}">
            Owner name:
            <nuxt-link v-if='item && item.owner_uuid' :to="localePath('/user/' + item.owner_uuid)">
    ...
    <ConfirmDialog v-model="confirm" title="Do you want to delete this item?" :loading="loadingDelete" @accept="confirmDelete" @cancel="confirm = false"></ConfirmDialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    confirmDelete(){
      ...
      this.$api.delete('/medical-record/' + this.deleteId).then(() => {
